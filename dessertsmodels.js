const desserts= [
  { name: 'Chocolate Cupcake', 
  price: 3, 
  image: 'https://sallysbakingaddiction.com/wp-content/uploads/2017/06/moist-chocolate-cupcakes-5.jpg', 
  flavors: ["Double Chocolate", "Dark Chocolate", "White Chocolate"]
},
  {name: "Funfetti Cookie", 
  price: 2, 
  image: 'http://thewoodandspoon.com/wp-content/uploads/2017/07/Last-Import-1-of-47-16.jpg', 
  flavors: ["Rainbow", "Fall Colors", "Red, White, Blue"]
  }, 
 
  {name: "Gummy Bears", 
  price: 3, 
  image: 'https://upload.wikimedia.org/wikipedia/commons/a/a6/Oursons_g%C3%A9latine_march%C3%A9_Rouffignac.jpg', 
  flavors: ["Rainbow", "Strawberry", "Peach"]
  }, 
  {name: "Bundt Cake", 
  price: 20, 
  image: 'https://butternutbakeryblog.com/wp-content/uploads/2019/02/red-velvet-bundt-cake-1.jpg', 
  flavors: ["Red Velvet", "Vanilla", "CarrotCake"]
  }, 
  {name: "Banana Bread", 
  price: 5, 
  image: 'https://www.simplyrecipes.com/thmb/n0haOVlgXcM8Sr53p3C-U53YP2U=/4399x3299/smart/filters:no_upscale()/Simply-Recipes-Easy-Banana-Bread-LEAD-2-2-63dd39af009945d58f5bf4c2ae8d6070.jpg', 
  flavors: ["Original", "With Nuts", "With Frosting"]
  }, 
 
  {name: "Pie", 
  price: 5, 
  image: 'https://www.supermarketperimeter.com/ext/resources/2021/08/19/0819---holiday-pies.jpg?t=1629384170&width=1080', 
  flavors: ["Pecan", "Apple", "Key Lime"]
  }, 

  { name: 'Cinnamon Rolls', 
  price: 5 , 
  image: 'https://www.thespruceeats.com/thmb/roPAlvp567wXt4sORxzCP17s7o0=/940x0/filters:no_upscale():max_bytes(150000):strip_icc():format(webp)/quick-cinnamon-rolls-3053776-81543bd7548c4b7fa95b8c51aaec316d.jpg', 
  flavors: ["Original", "Cream cheese", "Strawberry"]

  },
  { name: 'Ice Cream', 
  price: 3 , 
  image: 'https://www.awesomealpharetta.com/wp-content/uploads/2020/06/ice_cream_cones_blog.jpg', 
  flavors: ["Vanilla", "Chocolate", "Cookies and cream", "Mint chocolate"]

  },

]

module.exports = desserts